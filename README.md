...
## Ссылка из оформления заказа

Если в Playerok у лота есть поле `Ссылка на Discord сервер`, бот сначала берёт ссылку оттуда и проверяет её без лишних сообщений. Для подходящей ссылки он отправляет покупателю название найденного сервера и просит подтвердить выдачу. Если поле пустое или ссылка не проходит проверку, бот молчит и ждёт ссылку в чате. Ссылка, присланная в чат, проверяется с ответом об ошибке, если сервер не подходит.
...
Логи выводятся в консоль и сохраняются в `logs/neverplayerok.log`. В консоли отображаются запуск, авторизация, события Playerok, заказы, проверки Discord, вызовы NeverBoost API, плагины, Telegram и автоматизация. API-ключи, cookies и токены намеренно не выводятся.
