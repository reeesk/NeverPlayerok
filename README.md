...
В Telegram-панели нажмите `🆕 Обновить бота`. Бот проверит ветку `main` на GitHub и предложит установить новую версию. Пользовательские данные не перезаписываются: сохраняются `data/`, `plugins/`, `logs/` и `.venv`.

Для ручного обновления используйте:
